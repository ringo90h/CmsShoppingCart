    ...
    public class ShopController : Controller
    {
        ...
        public ActionResult AddProduct(ProductVM model, HttpPostedFileBase file)
        {
            ...
            using(Db db = new Db())
            {
                ...
                //FirstOrDefault = 처음 개체나 개체가 없으면 기본값 출력
        ...
        public ActionResult Products(int? page, int? catId)
        {
            ...
            using(Db db = new Db())
            {
                ...
                //Populate categorie select list
        ...
        public void SaveGalleryImages(int id)
        {
            ...
            foreach (string fileName in Request.Files)
            {
                ...
                if ( file != null && file.ContentLength > 0)
                {
                    ...
                    string originalPath = Path.Combine(originalDirectory.ToString(), "Products\\" + id.ToString() + "\\Gallery");
                    string thumbPath = Path.Combine(originalDirectory.ToString(), "Products\\" + id.ToString() + "\\Gallery\\Thumbs");

                    // Set image paths
                    var originalFilename = string.Format("{0}\\{1}", originalPath, file.FileName);
                    var thumbFilename = string.Format("{0}\\{1}", thumbPath, file.FileName);

                    // Save original and thumb

                    file.SaveAs(originalFilename);
                    WebImage img = new WebImage(file.InputStream);
                    img.Resize(200, 200);
